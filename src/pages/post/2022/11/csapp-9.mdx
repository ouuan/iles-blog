...
合并时需要知道上一个 block 的信息，这可以通过在 block 尾部添加一个与 header 内容相同的 footer 来实现，这被称作使用 *boundary tags*。由于只有 free block 需要 footer，可以省去 allocated block 的 footer 来节省空间。
...
在 C 中，由于没有类型信息，可能会将其他类型的数据视作对 block 的引用，导致不可达的 block 被视作可达，所以 C 语言的 garbage collection 只能是 conservative 的。
