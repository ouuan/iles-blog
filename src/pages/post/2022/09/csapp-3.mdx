...
image: /images/2022/09/csapp-3.png
tags:
    - csapp
    - 计算机系统
    - 学习笔记
...
可以通过 `gcc -S` 生成汇编代码，通过 `gcc -Og` 来应用不影响调试（不影响汇编代码可读性）的优化。
...
每种类型都有一个用在汇编指令中的后缀，表示 operand 的类型，例如 `movb`、`movw`、`movl`、`movq`。`l` 既用于 double word 也用于 double precision，但整数和浮点数涉及的指令不同，所以不会有歧义。
...
3.  Memory，完整形态的代码为 `Imm(rb, ri, s)`，表示 `M[Imm + R[rb] + R[ri] * s]`，其中 $s \in \{1, 2, 4, 8\}$，例如 `2(%rax, %rbx, 4)` 表示 memory 中地址为 `2 + %rax + 4 * %rbx` 的值；`Imm`、`rb`、`, ri, s` 分别可以省略，例如 `Imm`、`(rb)`、`Imm(, ri, s)`。
...
-   `imulq r/m64`: 计算 operand 和 `R[%rax]` 作为 signed integer 相乘而不截去高位的结果，存在 `R[%rdx]:R[%rax]` 中。
...
-   `cqto`: 将 `R[%rax]` 高位填符号位放在 `R[%rdx]:R[%rax]`（也就是用 `R[%rax]` 的符号位填满 `R[%rdx]`）。
...
若商溢出了，则会触发 divide error 异常，所以一般来说被除数会是 64 位整数（`idivq` 之前可以用 `cqto` 来设置 `R[%rdx]`，`divq` 之前会将 `R[%rdx]` 置为全零），否则很可能溢出而触发异常。

## Control

<Card type="hint" title="“status flag” vs “condition code”">
    在 CS:APP 中 `CF`/`ZF`/`SF`/`OF` 被叫做“condition code”；而在 Intel Manual
    中，`CF`/`ZF`/`SF`/`OF` 被叫做“status flag”，“condition code”指的是 status
    flags 的组合。下文采用 Intel 的叫法。
</Card>

### Status Flags

status flag 中存储了最近一次运算的状态，常用的 status flag 有四个：
...
调用 procedure 的核心是在 [push/pop stack](#push-pop-stack) 一节中介绍过的 runtime stack。大体上来讲，stack 会分成一堆 frame，栈顶的 frame 为当前 procedure 的相关数据，从栈顶到栈底的各个 frame 依次放着调用链上的各个 procedure，在调用一个 procedure 时会将相关数据压入栈中，返回时再弹出。
...
在 x86-64 中，进行 data Alignment 可以提升程序效率。具体要求为，任何基础类型（主要是结构体内）的地址必须是其长度的倍数。
...
但是，有的语言可能需要能够执行动态生成的指令，这样的话就不能禁止执行 stack 中的指令。

## Floating-Point Code

这部分内容基于 AVX2 指令集，可以在编译时指定 `-mavx2`。如果不支持 AVX2 也会是类似的，但指令名称等可能不同。
...
<Card type="question" title="gcc 使用的浮点数精度转换指令">
CS:APP 里提到 gcc 一般不使用这两条指令，而是会使用另外的指令，且没有明确的理由这样做 (p298)，但是我使用 gcc 12.2.0 编译出来的就是使用这两条指令 🤔。

浅搜了一下，没找到相关的 gcc commits 或者讨论，也没试着对 gcc version bisect。
...
-   `sqrtss xmm/m32, xmm, xmm`: 将 $\sqrt{S_1}$ 存在 $D$ 的低位，高位从 $S_2$ 复制
