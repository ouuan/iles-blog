...
如果 TLB 满了，添加新的 entry 时需要替换掉一个旧的，需要根据 replacement policy 进行选择。

[讨论 swapping 的 replacement policy 时](#beyond-physical-memory-policies) 会再深入探讨。这里简单介绍两种 policy：

-   *least-recently-used* (LRU): 认为越久没使用的未来也越不会使用，基于此进行替换。
-   random: 随机替换，更加容易实现，而且可以避免在 corner case 下挂掉。例如，如果循环访问 $n+1$ 个 page，而 TLB size 是 $n$，则 LRU 会产生大量 miss，而 random 不会。
...
<Card title="“an array stored on disk and cached in memory”">

CS:APP 将 virtual memory 称作 “an array stored on disk and cached in memory”，导致我一直都没完全理解（，感觉还是 OSTEP 好（
